...
/*
        uint8_t relay_packet[34]; //make complete packet
        //u_int8_t gatemac[6];
        //Mac mymac;
        uint16_t iptype = htons(0x0800);
        struct iphdr
        memset(relay_packet,0,34);
        //-------ether
        memcpy(relay_packet,&gatemac,6);//gatewaymac
        memcpy(relay_packet+6,&tm,6);//targetmac(victim)
        memcpy(relay_packet+12,&iptype,2);
        //-------ip
        memcpy(relay_packet+28,&s_ip,4); //targetip(victim)
        memcpy(relay_packet+38,&gate_t_ip,4);
*/
}
